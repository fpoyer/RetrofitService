...
public class GithubRepositoryListSyncService extends IntentService {
    ...
    public void getFreshData(final GithubRepositoryManager manager) {
        // Create a very simple REST adapter which points the GitHub API endpoint.
        GitHubClient client = ServiceGenerator.createService(GitHubClient.class);

        // Fetch and print a list of the repos of this owner.
        Call<List<Repository>> call =
                client.repositories(manager.getUSERNAME());
        ...
        call.enqueue(new Callback<List<Repository>>() {
                         ...
                         public void onResponse(Call<List<Repository>> call, retrofit2.Response<List<Repository>> response) {
                             if (response.isSuccessful()) {
                                 ...
                                 String msg = "error response code from server: " + response.code();
                                 Log.e(TAG, msg);
                                 List<Repository> fakeList = new ArrayList<Repository>();
                                 Repository repo1 = new Repository();
                                 repo1.setName("Fake Error Repository!!");
                                 fakeList.add(repo1);
                         ...
                         public void onFailure(Call<List<Repository>> call, Throwable t) {
                             ...
                             List<Repository> fakeList = new ArrayList<Repository>();
                             Repository repo1 = new Repository();
                             repo1.setName("Fake Failure Repository!!");
                             fakeList.add(repo1);
                             manager.setLastResult(fakeList);
                         }
                     }
